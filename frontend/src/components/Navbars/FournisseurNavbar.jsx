...
function AdminNavbar(props) {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        const user_id = localStorage.getItem('user_id');
        const response = await axios.get(`http://127.0.0.1:8000/api/getAllMotifs?user_id=${user_id}`);
    ...
    <>
      ...
      <DropdownMenu className="dropdown-navbar" right tag="ul">
        {notifications.map((notification, index) => (
          <li key={index}>
            <DropdownItem>
              La facture n° {notification.num_fact} est rejétée à cause du {notification.motifs}
            </DropdownItem>
          </li>
        ))}
      </DropdownMenu>
  ...
  <p>{user.lastname}</p>
      ...
      <DropdownItem className="nav-item">
        <Link to="/Fournisseur/profile" style={{ color: 'grey' }}>
          Profile
        </Link>
      </DropdownItem>
    </NavLink>
