...
const CommissionDistribution = () => {
  ...
  useEffect(() => {
    const fetchChargesLocatives = async () => {
      try {
        ...
        const { fournisseur,commission,id_fiscale,date_rec,date_facture,montant,delai_paiement,num_facture,devise} = response.data;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/commission-distribution/${id}`,
        formData,
        {
          headers: {
