...
function FacturesList() {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/devisee/${selectedFactureId}/motif`, {
        motifs: motifs,
      });
  ...
  useEffect(() => {
    ...
    const fetchFactures = async () => {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/api/current-date-facturesd?user_id=${user_id}`);
  ...
  const handleDelete = async (id) => {
    try {
        await axios.delete(`http://127.0.0.1:8000/api/devisee/${id}`);
...
const handleValidation = async (id) => {
  try {
    ...
    const response = await axios.post(`http://127.0.0.1:8000/api/devisee/${id}/validation`, {
      [validationType]: 'oui',
    });
    if (response.data.success) {
...
const handleSubmitr = async (id) => {
  try {
    const response = await axios.post(`http://127.0.0.1:8000/api/devisee/${id}/motif`, {
      motifs: "aucune",
    });
    if (response.data.success) {
      setModalOpen(false);
...
const exportToExcel = () => {
  const jsonData = factures.map((facture) => ({
    ...
    "Date réception": facture.dateReception,
    Devise: facture.devise,
    montant: facture.montant,
    "Numéro facture": facture.numeroFacture,
    "Delai de paiement": facture.delaiPaiement,
    objet: facture.objet,
        ...
        <Card>
          ...
          <Input
              ...
              >
                ...
                <option value="idFiscale">ID fiscale</option>
              </select>
              <Input
                type="text"
...
>
  <FaCheck />
    ...
    <Link className="btn btn-black btn-sm btn-icon-split" to={`/Agent/consulterdevise/${facture.id}`}>
    <span className="icon">
        <i className="fas fa-eye"></i> {/* Utilisez ici l'icône appropriée pour un œil */}
    </span>
</Link>
    <Link className="btn btn-info btn-sm btn-icon-split" to={`/Agent/devisee/${facture.id}`}>
      <span className="icon">
