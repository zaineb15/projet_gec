...
const Fiscalité= () => {
  ...
  useEffect(() => {
    const fetchChargesLocatives = async () => {
      try {
        ...
        const { beneficiaire,num_po,dossier_fiscalite,date_ordre_paiement,montant,devise,date_reception} = response.data;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/fiscalite/${id}`,
        formData,
        {
          headers: {
