...
function FacturesList() {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/charges-locatives/${selectedFactureId}/motif`, {
        motifs: motifs,
      });
  ...
  useEffect(() => {
    const fetchFactures = async () => {
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/api/current-date-facturescl?user_id=${user_id}&user_profil=${user_profil}`);
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`http://127.0.0.1:8000/api/charges-locatives/${id}`);
  ...
  const handleValidation = async (id) => {
    try {
      ...
      // Envoyer la requête avec le type de validation approprié
      const response = await axios.post(`http://127.0.0.1:8000/api/charges-locatives/${id}/validation`, {
  ...
  const handleSubmitr = async (id) => {
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/charges-locatives/${id}/motif`, {
        motifs: "aucune",
      });
      if (response.data.success) {
        setModalOpen(false);
    ...
    // Filtrer les factures en fonction des numpos
    const filteredFactures = factures.filter((facture) => {
      ...
      return numpoMatched && idfiscaleMatched && directionMatched;
    });
    const exportToExcel = () => {
      ...
      a.href = url;
      a.download = "Charges_locatives.xlsx";
        ...
        <Card>
          ...
          <InputGroup className="mb-3">
              ...
              >
                ...
                <option value="numero_po">Numéro PO</option>
              </select>
              <Input
                type="text"
              ...
              <Input
                type="text"
                placeholder="chercher ID fiscale..."
...
>
  <FaCheck />
    ...
    <Link className="btn btn-black btn-sm btn-icon-split" to={`/Agent/consulterchargelocative/${facture.id}`}>
    <span className="icon">
        <i className="fas fa-eye"></i> {/* Utilisez ici l'icône appropriée pour un œil */}
    </span>
</Link>
                          <Link className="btn btn-info btn-sm btn-icon-split" to={`/Agent/charges-locatives/${facture.id}`}>
                            <span className="icon">
