...
function FacturesList() {
  ...
  useEffect(() => {
    const fetchChargesLocatives = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/current-date-facturescs/${id}`);
        const { fournisseur,objet,dossier_fiscalite,date_ordre_paiement, numero_op,montant,devise,date_reception} = response.data;
        setFournisseur(fournisseur);
        setMontant(montant);
        setDevise(devise);
        setDateReception(date_reception);
        setNumeroOP(numero_op);
        setDateOrdrePaiement(date_ordre_paiement);
        setDossierFiscalite(dossier_fiscalite);
        setObjet(objet);
      } catch (error) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/charges-sociales/${id}`,
              formData,
        {
          headers: {
