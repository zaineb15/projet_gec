...
const Devise = () => {
  ...
  useEffect(() => {
    const fetchChargesLocatives = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:8000/api/current-date-facturesd/${id}`);
        const { fournisseur,idFiscale,dateFacture,montant,delaiPaiement,nature3wm,numeroFacture,devise,dateReception} = response.data;
        setFournisseur(fournisseur);
        setIdFiscale(idFiscale);
        setDateFacture(dateFacture);
        setMontant(montant);
        setDelaiPaiement(delaiPaiement);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/devisee/${id}`,
        formData,
        {
          headers: {
