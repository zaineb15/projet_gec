...
const Steg = () => {
  ...
  useEffect(() => {
    const fetchChargesLocatives = async () => {
      try {
        ...
        const { fournisseur,id_fiscale, numero_po,direction,date_facture,montant,delai_paiement,nature_3wm,structure_ordinatrice,numero_facture,devise,date_reception} = response.data;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/steg/${id}`,
        formData,
        {
          headers: {
