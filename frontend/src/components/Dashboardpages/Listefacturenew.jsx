...
function FacturesList() {
  ...
  const handleSubmit = async () => {
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/api/facture/${selectedFactureId}/motif`,
        {
          motifs: motifs,
        }
      );
    ...
    const fetchFactures = async () => {
      try {
        ...
        const response = await axios.get(
          `http://127.0.0.1:8000/api/facturenew?user_profil=${user_profil}`
        );
  ...
  const handleValidation = async (id) => {
    ...
    if (!validationType) {
      ...
      const response = await axios.post(
        `http://127.0.0.1:8000/api/facture/${id}/validation`,
        {
          [validationType]: 'oui',
        }
      );
  ...
  const handleSubmitr = async (id) => {
    try {
      const response = await axios.post(
        `http://127.0.0.1:8000/api/facture/${id}/motif`,
        {
          motifs: "aucune",
        }
      );
  ...
  const filteredFactures = factures.filter((facture) => {
    ...
    return numeroFactureMatched && numpoMatched;
  });
  const exportToExcel = () => {
    const jsonData = factures.map((facture) => ({
      ...
      "Date facture": facture.date_fact,
      montant: facture.montant,
      objet: facture.objet,
      "Numéro facture": facture.num_fact,
    ...
    a.href = url;
    a.download = "Facture_fournisseur_3wm_TND.xlsx";
