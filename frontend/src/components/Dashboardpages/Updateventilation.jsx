...
const Ventilation = () => {
    ...
    const fetchChargesLocatives = async () => {
      try {
        ...
        const {
          ...
          objet,
          devise,
          date_reception,ventilation_direct,ordre_paiement
        } = response.data;
        ...
        setVentilationDirect(ventilation_direct);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`http://127.0.0.1:8000/api/ventilation/${id}`,
        formData,
        {
          headers: {
